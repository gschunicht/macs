	...
	<head>
		...
		<script>
			$(document).ready(function () {
				...
				setupMenu();
				var mach_id = "%";//<?php if (isset($_GET["mach_id"])) {echo htmlspecialchars($_GET["mach_id"]);}else {echo "%"; }?>;
			...
			function makeMachGrid (mach_id){
				 ...
				 var gridMach = $("#gridMach").kendoGrid({
					...
					columns: [
						...
						template: "#= templateActive(active) #",field: "active",
						title: "Active",
						width: 9 
					},{
						field: "name",
				...
				$("#gridMach").data("kendoGrid").dataSource.filter({
				field: "active",
				operator: "eq",
				value: "true"
				});
...
<div id="master" class="Content">
	<div id="menu"></div>
    <div id="gridMach"><h2>MACS Machines: Select row to add users<a id="titleSelected" class="SelectedTitle"></a></h2></div>
